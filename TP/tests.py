...
def test_method1(): ##1.	Dos textos idénticos deben dar similitud >= 99%
    ...
    assert mainFunctions.JaccardScore(doc1.text, doc2.text) > 99
...
def test_method2():##Dos textos completamente diferentes dan una similitud menor al 10%
    ...
    assert mainFunctions.JaccardScore(s1, s2) < 10
...
def test_method3():##Una cita sin referencia correspondiente penaliza el score final
    ...
    assert mainFunctions.JaccardScore(s1, s1CopyWithoutCitation) > mainFunctions.JaccardScore(s1, s1CopyWithCitation)
...
def test_method5():##Dos oraciones identicas en textos distintos aumentan la similitud
    ...
    assert mainFunctions.JaccardScore(doc1.text + addedSentence, doc2.text + addedSentence) > mainFunctions.JaccardScore(doc1.text, doc2.text)
...
def test_method6():##una copia parcial de una oracion penaliza la oracion, aunque menos que si la copia fuera total
    ...
    assert mainFunctions.JaccardScore(s1, s1PartialCopy) > 0 and \
           (mainFunctions.JaccardScore(s1, s1FullCopy) > mainFunctions.JaccardScore(s1, s1PartialCopy))
