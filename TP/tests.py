...
def test_method2():##Dos textos completamente diferentes dan una similitud menor al 10%
    ...
    assert mainFunctions.PlagiarismScorePlainText(s1, s2) < 10
...
def test_method3():##Una cita sin referencia correspondiente penaliza el score final
    ...
    assert mainFunctions.PlagiarismScorePlainText(s1, s1CopyWithoutCitation) > mainFunctions.PlagiarismScorePlainText(s1, s1CopyWithCitation)
...
def test_method5():##Dos oraciones identicas en textos distintos aumentan la similitud
    ...
    assert mainFunctions.PlagiarismScorePlainText(doc1.text + addedSentence, doc2.text + addedSentence) > mainFunctions.PlagiarismScorePlainText(doc1.text, doc2.text)
...
def test_method6():##una copia parcial de una oracion penaliza la oracion, aunque menos que si la copia fuera total
    ...
    s1PartialCopy = "Ironman salvó al hombre que estaba atrapado dentro del edificio en llamas."
    s1FullCopy = s1

    assert mainFunctions.PlagiarismScorePlainText(s1, s1PartialCopy) > 0 and \
           (mainFunctions.PlagiarismScorePlainText(s1, s1FullCopy) > mainFunctions.PlagiarismScorePlainText(s1, s1PartialCopy))
